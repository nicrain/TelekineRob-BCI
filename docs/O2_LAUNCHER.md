...
| M2 | 总控网页 | 侧边栏骨架 + 状态轮询 + iframe | ⬜ |
| M3 | 启动系统链路 | 起 WSL → 检测 → 同步 launcher+桥文件（自同步）→ 起前后端 → 就绪检测 → 解灰 | ⬜ |
| M4 | 设备连接链路 | 连/断 Headband/Hybrid（跑桥+验 LSL）/ Thymio（usbipd+验 ttyACM0） | ⬜ |
| M5 | 状态显示 | 设备/系统状态 + 人话错误 | ⬜ |
